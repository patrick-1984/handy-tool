...
pub fn send_transcription_input(app: &AppHandle, binding_id: &str, source: &str) {
    // Hold-to-talk bindings need a matching release; a one-shot external press
    // would start a recording that no press can ever stop.
    if binding_id == "transcribe_ptt" {
        warn!("Ignoring external press for hold-based binding 'transcribe_ptt' (from {source})");
        return;
    }
