...
pub fn get_app_dir_path(app: AppHandle) -> Result<String, String> {
    // Portable-aware (T-114): report the folder the app actually uses so a
    // portable launch shows its own `data\` dir, not %APPDATA%\pr.handy.
    let app_data_dir = crate::portable::resolve_app_data_dir(&app)?;
...
pub fn get_log_dir_path(app: AppHandle) -> Result<String, String> {
    ...
    // is active, not %LOCALAPPDATA%\pr.handy\logs.
    let log_dir = crate::portable::resolve_log_dir(&app)?;
...
pub fn open_log_dir(app: AppHandle) -> Result<(), String> {
    // Portable-aware (T-114 gap #2): open the same folder logs are actually
    // written to — see get_log_dir_path.
    let log_dir = crate::portable::resolve_log_dir(&app)?;
