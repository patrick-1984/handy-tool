...
impl AudioRecordingManager {
    ...
    /// Build the chunked-Opus recording target for a new recording, or `None`
    /// when crash-safe recording is disabled. Chunk files use the caller-provided
    /// recording timestamp (`handy-{ts}-chunk-N.opus`); in-progress chunks carry
    /// a `-temp` suffix so startup recovery can tell them apart from finalized
    /// ones. The same `ts` is used by the history layer to point at the glued
    /// `handy-{ts}.opus`.
    fn chunk_target_for_new_recording(&self, ts: u64) -> Option<StartParams> {
        let settings = get_settings(&self.app_handle);
    ...
    pub fn clear_segment_callback(&self) {
        if let Some(rec) = self.recorder.lock().unwrap().as_ref() {
            rec.clear_segment_callback();
        }
    }

    /// Set a callback that fires when a recording chunk is finalized (including
    /// the final chunk on stop). Used by the chunked transcription pipeline.
    pub fn set_on_chunk_callback<F>(&self, cb: F)
    where
        F: Fn(ClosedChunk) + Send + Sync + 'static,
    {
        if let Some(rec) = self.recorder.lock().unwrap().as_ref() {
            rec.set_closed_chunk_callback(cb);
        }
    }

    /// Clear the closed-chunk callback.
    pub fn clear_on_chunk_callback(&self) {
        if let Some(rec) = self.recorder.lock().unwrap().as_ref() {
            rec.clear_closed_chunk_callback();
        }
    }

    /// Cancel any ongoing recording without returning audio samples. Discards
    /// the in-progress chunk and any finalized chunk files for this take.
    pub fn cancel_recording(&self) {
        ...
        if let RecordingState::Recording { .. } = *state {
            *state = RecordingState::Idle;
            ...
            if let Some(rec) = self.recorder.lock().unwrap().as_ref() {
                let _ = rec.cancel(); // Discard audio + chunk files
            }
