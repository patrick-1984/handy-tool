...
async function runVADExample() {
  try {
    ...
    if (!fs.existsSync(vadParams.vad_model)) {
      console.log(
        "⚠️  VAD model not found. Please download the VAD model first:",
      );
    ...
    console.log(
      `  - Min speech duration: ${vadParams.vad_min_speech_duration_ms}ms`,
    );
    console.log(
      `  - Min silence duration: ${vadParams.vad_min_silence_duration_ms}ms`,
    );
    console.log(
      `  - Max speech duration: ${vadParams.vad_max_speech_duration_s}s`,
    );
    ...
    if (speedup > 1) {
      ...
      console.log(
        `ℹ️  Traditional approach was ${(1 / speedup).toFixed(2)}x faster in this case.`,
      );
    }
  } catch (error) {
